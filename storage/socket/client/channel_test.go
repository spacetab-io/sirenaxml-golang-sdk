...
func TestNewChannel(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		c, err := NewChannel(
			l,
			SetClientID(12),
			SetClientPrivateKey(conf.ClientPrivateKey),
			SetClientPrivateKeyPassword(conf.ClientPrivateKeyPassword),
			SetClientPublicKey(conf.ClientPublicKey),
			SetServerPublicKey(conf.ServerPublicKey),
			SetEnvironment(conf.Environment),
			SetIp(conf.Ip),
			SetMaxConnections(conf.MaxConnections),
